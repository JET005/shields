...
const ipPeriod = 600;  // 10 min, in seconds
...
function setRoutes(server) {
  server.handle(function monitorHandler(req, res, next) {
    ...
    const ip = req.url.split(/[\/-]/).slice(0, 3).join('');
    const ipHitsInCurrentPeriod = ipHits.get(ip) || 0;
    if (ipHitsInCurrentPeriod > maxHitsPerPeriod
      && !whitelist.test(ip)) { bannedIps.add(ip); }
    if (bannedIps.has(ip)) {
      res.statusCode = 429;
    ...
    ipHits.set(ip, ipHitsInCurrentPeriod + 1);
